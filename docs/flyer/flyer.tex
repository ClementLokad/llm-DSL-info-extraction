...
\begin{center}
    {\Huge \bfseries \color{lokadblue} PSC INF01 X24: LLM for Information Extraction in a DSL: Case of Envision (LOKAD)}
\end{center}


\vspace{0.6cm}

\begin{tcolorbox}[colback=lokadblue!5!white,colframe=lokadblue,title=\centering \bfseries The INF01 Team]
    \centering
...
\begin{center}
    ...
    \begin{tikzpicture}[
        ...
        \draw[arrow] (mainllm.south) -- (final.north);
        
        \draw[arrow] (final.west) -- (grader.east);
        \draw[arrow] (grader.north) -- (questions.south);
...
\begin{tcolorbox}[colback=white,colframe=lokadblue!40,title=\bfseries Project Summary]
The INF01 project provides an intelligent assistant designed for the analysis of proprietary code written in \textbf{Envision} (Lokad's DSL). The project addresses the opacity of massive codebases by using an agentic architecture capable of:
\begin{itemize}
    \item \textbf{Planning} dynamic multi-step search strategies.
    \item \textbf{Extracting} information via RAG, Static Analysis, and Dependency Graphs.
    \item \textbf{Explaining} the code's internal workings with a verifiable source of truth.
\end{itemize}
This approach enables increased autonomy for new developers and flattens the learning curve for complex business tools.
\end{tcolorbox}

\vfill
\begin{center}
    \small \color{gray} Access full documentation \& source code: \\
    \small \color{gray} Documentation: \textbf{\url{https://kpihx.github.io/envision-copilot-presentation/}} \\
    \small \color{gray} Source code: \textbf{\url{https://github.com/ClementLokad/llm-DSL-info-extraction}}
\end{center}
