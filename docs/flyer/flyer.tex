...
\begin{center}
    ...
    \begin{tikzpicture}[
        ...
        % Processing
        \node[box, right=1.8cm of grep] (distill) {Distillation \\ \labelstyle{LLM}};
        \node[box, below right=2cm and 1.8cm of grep] (mainllm) {Main LLM \\ \labelstyle{drafts the answer}};
        \node[box, below=1.8cm of planner] (final) {Final Answer \\ \labelstyle{evidence-based output}};
        ...
        \draw[arrow] (planner.south) -- (final.north) node[pos=0.5, left] {\labelstyle{else}};
        
        \draw[arrow] (final.west) -- (grader.east);
        \draw[arrow] (grader.south) -- (benchmark.north);

        % Loops
        \draw[arrow, dashed] (mainllm.west) .. controls +(-2.5, 0) and +(1, -2.5) .. node[below, pos=0.2, font=\tiny] {feedback loop} (planner.south east);
