...
class DSLQuerySystem():
    ...
    def interactive(self, verbose=False):
        ...
        # Simple check in case we use MainLinearPipeline (even tho it's kinda outdated)
        is_agentic = isinstance(self.pipeline, MainAgenticPipeline)
        if is_agentic:
            input_state = AgentGraphState( # Initialize the AgentGraphState to be kept throughout the session
                question="",
                reference_answer="",
                knowledge_bank=[],
                execution_history=[],
...
EXAMPLES:
  ...
  python main.py --help                               # Show full help message
    ...
    parser.add_argument(
        "--agent", "-a",
        choices=["mistral", "groq", "qwen", "qwen-ssh", "deepseek-v3", "deepseek-r1", "sonnet", "haiku"],
