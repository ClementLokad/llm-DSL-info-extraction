...
class MainLinearPipeline(BasePipeline):
    ...
    def retrieve_documents(self, state):
        ...
        #Fusion Mode: Retrieve via the embedding of sub_queries generated from the query           
        elif self.config_manager.get('query_transformer.query_transformer_mode', None) == "fusion":
           
            query_transformer = FusionQueryTransformer(self.agent, self.rate_limit_delay, self.console)
            transformed_question_list = query_transformer.transform(question)
            
            if state["verbose"]:
                self.console.print(f"[dim]Raw answer from LLM for decomposition of the query : {', '.join(transformed_question_list)}[/dim]")
        ...
        #HyDe Mode: Retrieve via the embedding of virtual "golden" document generated from the query
        elif self.config_manager.get('query_transformer.query_transformer_mode', None) == "hyde":
            
            query_transformer = FusionQueryTransformer(self.agent, self.rate_limit_delay, self.console)
            transformed_question_list = query_transformer.transform(question)
            
            if state["verbose"]:
                self.console.print(f"[dim]Virtual Documents generated by LLM from the query : {', '.join(transformed_question_list)}[/dim]")
