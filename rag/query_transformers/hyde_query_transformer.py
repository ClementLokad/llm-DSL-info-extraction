...
class HydeQueryTransformer(BaseQueryTransformer):
    ...
    def transform(self, query : str, verbose: bool = False) -> list[str]:
        if self.index_type == "summaries":
            self.hyde_prompt = f"Act as an Envision DSL expert. Generate {self.generated_instances_amount}"
            "concise technical summaries (max 150 words) of a script answering this query: {query}." 
            "Your response must only be the juxtaposition of these summaries, with each one separated by a $"
            "character. Do not add any preamble, explanation, or other text \n"
